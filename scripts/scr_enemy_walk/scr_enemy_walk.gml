...
{
    var _movespeed, backtoroom;
    
    _movespeed = grounded ? (movespeed + (clamp(global.heatmeter ? global.stylethreshold : 0, 0, 3.25) * sign(movespeed))) : 0;
    ...
    image_speed = 0.35 + ((global.heatmeter ? global.stylethreshold : 0) * 0.05);
...
{
    var _movespeed = (grounded ? (movespeed + (clamp((global.heatmeter ? global.stylethreshold : 0), 0, 3.25)) * sign(movespeed)) : 0)
    if (grounded || (grounded && (!(place_meeting(x, y, obj_platform)))) || (!(place_meeting(x, y, obj_cottonplatform))))
    {
        if ((!(place_meeting(x, (y + 1), obj_railh))) && (!(place_meeting(x, (y + 1), obj_railh2))))
            hsp = image_xscale * _movespeed
