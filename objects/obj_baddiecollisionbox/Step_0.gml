...
if (instance_exists(baddieID) && place_meeting(x, y, obj_player) && !obj_player.cutscene && obj_player.state != states.hurt)
{
	if (baddieID.state != baddiestates.grabbed)
	{
		with (obj_player)
		{
			...
			if (instance_exists(other.baddieID) && state == states.cotton && sprite_index == spr_cottonattack)//spr_cotton_attack)
			{
				with (other.baddieID)
				{
					cottoned = true;
...
if (instance_exists(baddieID) && place_meeting(x, y, obj_player) && (!obj_player.cutscene) && obj_player.state != states.hurt)
{
    if (baddieID.state != states.boxxedpep && baddieID.state != states.meteorpep)
    {
        with (obj_player)
        {
            ...
            if (instance_exists(other.baddieID) && (place_meeting(x, (y + 1), other.baddieID) || place_meeting(x, (y + vsp), other.baddieID)) && y != other.baddieID.y && (!attacking) && (state == states.jump || state == states.grab) && sprite_index != spr_playerPZ_scooterspin && sprite_index != spr_playerPZ_scooterspinstart && vsp > 0 && other.baddieID.vsp >= 0 && sprite_index != spr_stompprep)
            {
                ...
                if (vsp > 0)
                {
                    ...
                    if key_jump2
                    {
                        ...
                        other.baddieID.hsp = (-other.baddieID.image_xscale) * 3
                        instance_create(x, (y + 50), obj_stompeffect)
                        other.baddieID.state = states.cheeseball
                        stompAnim = true
                        other.baddieID.image_index = 0
                        vsp = -14
            ...
            if (instance_exists(other.baddieID) && state == states.cotton && (sprite_index == spr_cottonattack || sprite_index == spr_cottonPizzano_drillup || sprite_index == spr_cottonPizzano_jumpwall || sprite_index == spr_cottonPizzano_wallclimb || sprite_index == spr_cottonPizzano_fastfall))
            {
                with (other.baddieID)
                {
                    cottoned = true
            ...
            if (instance_exists(other.baddieID) && other.baddieID.state != states.titlescreen && (!attacking) && state != states.tackle && state != states.hurt && (!y) < other.baddieID.y && (!y) > other.baddieID.y && (!grabbing) && other.baddieID.state != states.cheeseball && state != states.handstandjump && state != states.hookshot && state != states.pizzanoshoulderbash)
            {
                if (x != other.baddieID.x)
                {
                    other.baddieID.image_xscale = (-(sign(other.baddieID.x - x)))
                ...
                other.baddieID.hsp = (-other.baddieID.image_xscale) * 4
                other.baddieID.vsp = -4
                if (other.baddieID.state == states.slap || other.baddieID.state == states.hookshot)
                    other.baddieID.state = states.cheeseball
                if (other.baddieID.state == 87)
                {
                    stunned = 30
