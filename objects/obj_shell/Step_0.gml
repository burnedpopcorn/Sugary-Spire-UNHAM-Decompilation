...
else
{
    ...
    else if (historyUpModifiers.historyUpModifiers._key_combo_pressed(historyUpKey, self) && (!isAutocompleteOpen))
    {
        if (historyPos == array_length(history))
            savedConsoleString = consoleString
        historyPos = max(0, (historyPos - 1))
    ...
    else if (historyDownModifiers.historyDownModifiers._key_combo_pressed(historyDownKey, self) && (!isAutocompleteOpen))
    {
        if (historyPos < array_length(history))
        {
            historyPos = min(array_length(history), (historyPos + 1))
    ...
    else if keyboard_check_pressed(vk_return)
    {
        if (string_trim(consoleString) != "")
        {
            ...
            {
                var args = consoleString.consoleString._input_string_split(self)
                if (array_length(args) > 0)
                {
    ...
    else if (cycleSuggestionsModifiers.cycleSuggestionsModifiers._key_combo_pressed(cycleSuggestionsKey, self) || keyboard_check_pressed(vk_down))
    {
        if (array_length(filteredSuggestions) != 0)
        {
            var uncompleted = consoleString
    ...
    else if (cycleSuggestionsReverseModifiers.cycleSuggestionsReverseModifiers._key_combo_pressed(cycleSuggestionsReverseKey, self) || keyboard_check_pressed(vk_up))
    {
        if (array_length(filteredSuggestions) != 0)
        {
            suggestionIndex = (suggestionIndex + array_length(filteredSuggestions) - 1) % array_length(filteredSuggestions)
...
if (!is_undefined(activeMouseArgType))
{
    if (activeMouseArgType == states.normal)
        activeMouseArgValue = mouse_x
    else if (activeMouseArgType == states.titlescreen)
        activeMouseArgValue = mouse_y
    else if (activeMouseArgType == states.hookshot)
        activeMouseArgValue = device_mouse_x_to_gui(0)
    else if (activeMouseArgType == states.slap)
        activeMouseArgValue = device_mouse_y_to_gui(0)
    else if (activeMouseArgType == states.charge)
    {
        var instAtCursor = instance_position(mouse_x, mouse_y, all)
        if (instAtCursor != noone)
            activeMouseArgValue = instAtCursor
    ...
    else if (activeMouseArgType == states.cheesepep)
    {
