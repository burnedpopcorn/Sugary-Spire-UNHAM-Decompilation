...
if (place_meeting(x, y, _player) && (!(scr_solid(x, y))) && _player.state != states.fling && grabbedPlayer <= noone && waitTimer <= 0)
{
    ...
    with (grabbedPlayer)
    {
        if (state == states.uppercut || (state == states.jump && (sprite_index == spr_playerPZ_scooterspinstart || sprite_index == spr_playerPZ_scooterspin || sprite_index == spr_pizzano_wallclimbspin)))
        {
            movespeed = abs(hsp)
            if (move != 0)
                xscale = move
...
if (grabbedPlayer != noone)
{
    ...
    if (obj_player.input_buffer_jump < 8 || farBuffer >= 100)
    {
        with (grabbedPlayer)
        {
            state = states.jump
            ...
            if (sign(hsp) != 0)
                xscale = sign(hsp)
            obj_player.input_buffer_jump = 8
        }
        grabbedPlayer = -4
        waitTimer = 25
    }
